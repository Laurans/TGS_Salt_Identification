...
def VGGUnet(img_shape, start_filter=32, out_ch=1, bn=True, up=False, dro=0.5):
    ...
    def _up_block(input1, input2, filter, upconv=False):
        ...
        if upconv:
            ...
            x = Conv2DTranspose(filter, 3, strides=2,
...
def UNet(img_shape, out_ch=1, start_ch=64, depth=4, inc_rate=2., activation='relu',
         dropout=0.5, batchnorm=False, maxpool=True, upconv=True, residual=False, noise=False):
    def _conv_block(m, dim, acti, bn, res, do=0):
        n = Conv2D(dim, 3, activation=None, padding='same')(m)
        n = acti_layer(n, activation)
    ...
    def _level_block(m, dim, depth, inc, acti, do, bn, mp, up, res):
        if depth > 0:
            ...
            if up:
                m = UpSampling2D()(m)
                m = Conv2D(dim, 2, activation=None, padding='same')(m)
                m = acti_layer(m, acti)
            else:
                m = Conv2DTranspose(dim, 3, strides=2,
                                        activation=None, padding='same')(m)
                m = acti_layer(m, acti)
            n = Concatenate()([n, m])
    ...
    i = Input(shape=img_shape)
    a = GaussianNoise(0.001)(i) if noise else i
    o = _level_block(a, start_ch, depth, inc_rate, activation,
                     dropout, batchnorm, maxpool, upconv, residual)
    o = Conv2D(out_ch, 1, activation='sigmoid')(o)
    return Model(inputs=i, outputs=o)


def create_model(img_shape, vggunet, start_ch, depth=0, maxpool=True, upconv=True, residual=False, noise=False):
    K.clear_session()
    ...
    if vggunet:
        ...
        model = UNet(img_shape=img_shape, start_ch=start_ch, depth=depth,
                     batchnorm=True, residual=residual, maxpool=maxpool, upconv=upconv, noise=noise)
...
def fit(model, X_train, Y_train, x_valid, y_valid, output_name):
    ...
    checkpointer = ModelCheckpoint(
        output_name, save_best_only=True, verbose=1)

    reduce_lr = ReduceLROnPlateau(
        factor=0.1, patience=5, min_lr=1e-6, verbose=1)
    csvlog = CSVLogger('{}_log.csv'.format(output_name.split('.')[0]))

    results = model.fit(X_train, Y_train, validation_data=[x_valid, y_valid], batch_size=64, epochs=200,
